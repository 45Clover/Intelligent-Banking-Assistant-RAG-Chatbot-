...
async def chat_endpoint(payload: ChatPayload, authorization: str = Header(None)):
    ...
    try:
        ...
        llmResponse = out.get("response", "No response generated.")
        confidenceScore = out.get("confidence_score", 0.0)
        if confidenceScore <= 0.2 and llmResponse != "I cannot find that information in our current policies.":
            llmResponse = "[Warning: Low Confidence Score; Please consult our official banking policies or press 'Human Escalation' for a human consultant.]\n" + llmResponse
